...
public class MainActivity extends Activity {
	...
	final int LONG_CLICK_TIME = 500;
	final int STAND_DIST = 60;
	final int FLICK_DIST = 80;
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_POINTER_UP:
			...
			case MODE_CLICK:
				//	point
				if (Math.abs(opt.downX - x) < STAND_DIST && Math.abs(opt.downY - y) < STAND_DIST) {
	...
	class OnePointTouch {
		...
		public OnePointTouch(int _downX, int _downY) {
			...
			dragTimer.schedule(new TimerTask() {
				public void run() {
					if (Math.abs(moveX - downX) < STAND_DIST && Math.abs(moveY - downY) < STAND_DIST) {
						mode = MODE_DRAG;
